...
// ⚠️ Remplace par ta clé football-data.org (gratuite jusqu'à 10 requêtes/min) :
// https://www.football-data.org/client/register
const FOOTBALL_API_KEY = "REMPLACE_MOI";
const FOOTBALL_API_BASE = "https://api.football-data.org/v4";

// Compétitions couvertes (codes football-data.org). Ajuste selon ton abonnement API.
const COMPETITIONS = ["SA", "PL", "PD", "SA_BR", "CL"]; // exemples : Saudi Pro League*, Premier League, Liga, Brasileirão, C1
// *football-data.org ne couvre pas toutes les ligues (saoudienne, africaines) sur le plan gratuit —
// voir README.md pour les alternatives (API-Football via RapidAPI couvre davantage de championnats).

async function fetchTodayMatches() {
  if (FOOTBALL_API_KEY === "REMPLACE_MOI") {
    return getDemoMatches(); // données de démonstration tant que la clé API n'est pas configurée
  }
  try {
